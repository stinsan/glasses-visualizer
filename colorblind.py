...
def linear_lms(color):
    ...
    tmatrix = np.array([[0.31399022, 0.63951294, 0.04649755],
...
def lms_linear(color):
    ...
    invtmatrix = np.array([[5.47221206, -4.6419601, 0.16963708],
...
def protanopia(color):
    ...
    psimmatrix = np.array([[0, 1.05118294, -0.05116099],
                           [0, 1, 0],
                           [0, 0, 1]])  # lack of L cone
...
def tritanopia(color):
    ...
    tsimmatrix = np.array([[1, 0, 0],
                           [0, 1, 0],
                           [-0.86744736, 1.86727089, 0]])  # lack of m cone
...
def rg(img):
    """ Alters an image to Red/Green colorblindness"""
    start = time.time()
    ...
    for img_y in range(img_height):
        for img_x in range(img_width):
            ...
            pixels[img_x, img_y] = int(new_color[0]), int(new_color[1]), int(new_color[2])  # replace color

    diff = time.time() - start
    print('Colorblind time taken: {}'.format(diff))

    return img_copy


def by(img):
    """ Alters an image to Blue/Yellow colorblindness"""
    start = time.time()
